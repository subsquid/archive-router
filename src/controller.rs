        ...
        if let Some(goal) = assignment_map.get_mut(dataset) {
...

            {
                ...
                loop {
                    ...
                    let to_move = min(target_size - s_size, l_size - target_size);
...
#[cfg(test)]
mod tests {
    use crate::controller::{Config, ControllerBuilder};

    fn controller_builder<C: Config>() {
        let c1 = ControllerBuilder::<C>::new()
            .set_data_management_unit(100)
            .set_data_replication(4)
            .build();

        let c2 = {
            let mut builder = ControllerBuilder::<C>::new();
            builder.set_data_management_unit(100);
            builder.set_data_replication(4);
            builder.build()
        };
    }
}
